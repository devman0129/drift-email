...
const EditTypeDialog = (props) => {
  ...
  const handleSubmit = () => {
    props.onSubmit(chips, name, badgeColor, props.editData._id);
  };

  useEffect(() => {
    if (!isEmpty(props.editData)) {
      setChips(props.editData.keyword);
      setName(props.editData.name);
      setBadgeColor(props.editData.color);
    }
  }, [props.editData]);
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogContentText>
          Use this form to Edit a type to mark incoming emails. The Keyword can
